...
export default async function DeleteUser(
  ...
  try {
    ...
    if (user.membership_type != 'none' && !cancelled) {
      ...
      return res.redirect(`/api/stripe/cancel?callbackUrl=/api/member/delete?cancelled=true`)
    }

    await updateUser<User>(user.id, {
      status: 'delete',
      notes: `${user?.notes}\nUser requested deletion on ${new Date().toISOString()}`
    })

    return res.redirect(`/api/auth/signout?callbackUrl=/unauthorized?error=Your account has been deleted.`)
  } catch (e) {
    console.error(e)
    return res.redirect(
