...
export type MemberRelationBannerProps = FlexProps & {
  viewer: Member
...
export const MemberRelationBanner = chakra(
  ({ member, viewer, fontSize = ['xs', 'sm'], ...props }: MemberRelationBannerProps) => {
    ...
    const likesYou = member.likes?.some((l: UserLike) => String(l.like_id) == viewer?.id)
    const isYou = String(member.id) == viewer?.id

    const blocksYou = member?.blocked?.some((b) => String(b.blocked_id) == viewer?.id)
    const buddiesYou = member?.buddies?.some((b: UserBuddy) => b.user_id == viewer?.id)
    ...
    return (
      <>
        <Flex direction="row" justify="center" align="center" gap={1} p={1} w="full" {...props}>
          {blocksYou && (
            <>
