...
export const MemberMessageStats = memo(chakra(function MemberMessageStats({
  memberId,
  memberLevel,
  viewerLevel,
  boxSize = '20px',
  ...props }: MemberMessageStatsProps) {
  ...
  const { stats, loading: statsLoading } = useMessageStats(
    (memberLevel == MemberLevel.pledge || viewerLevel == MemberLevel.staff) ? memberId : null)
