...
export async function getFieldOptions<T = User>(
  ...
  return response
    ? (cache[key] = response!.meta!.options.choices)
    : [];
}
...
export async function updateUser(id: string, member: Partial<User>): Promise<User|null> {
  const adminClient = await getAdminClient();
...
export async function getApplicant(id: string) : Promise<Applicant|null> {
  const adminClient = await getAdminClient();
  const member: Applicant = await adminClient.items("users")
...
export async function findUser<T = Applicant>(email: string, fields = applicantFields) : Promise<T|null> {
  const adminClient = await getAdminClient();
