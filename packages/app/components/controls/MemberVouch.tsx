...
import { JsonFetcher, getAssetUrl, postJSON } from 'lib/utils'
...
export const MemberVouch = chakra(({ memberId, size = 'lg', ...props }: Props) => {
  ...
  const { data: voucher, isLoading, mutate } = swr<VouchingUser>(`/api/member/vouch/${memberId}`, JsonFetcher, {

  })

  const vouchForPledge = useCallback(() => {
    // add buddy
    postJSON<any, VouchingUser>(`/api/member/vouch/${memberId}`, {}).then((r) => {
      mutate(r.data, true)
      return reload()
    })
  ...
  }, [me, memberId, userLoading, setShowVouchButton, isLoading, voucher, level, myLevel])
      ...
      {showVouchButton && (
        <Popover>
          ...
          <PopoverContent color="text">
            ...
            <PopoverBody>
              <Text>
                When you vouch for someone, you put your reputation on the line.
                If this member is found to be disruptive, untrustworthy or
                otherwise not a good fit and needs to be banned, you may also get banned.
                Only vouch pledges you have met in person or have otherwise vetted and are
                certain they would make a great Brother.
              </Text>
              <Button my={2} w='full'
                size="lg"
                bg="primary"
